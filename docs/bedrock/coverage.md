...
| snow, both clients | 11 and 12 of about 700 | the first steps of a sprint off the walkway after dropping onto it, and a tick of sneaking down through it in leather boots |
...
| items, 1.26.51.1 | 58 of 3040 | Dolphin's Grace found a tick apart: the recorder moves the dolphin about 1.7 a tick, and at the engine's scan it is just outside the look box (which tick the client scans on is not in the capture); and a float32 unit (one ULP) in the height of a landing on the boat just left |
...
registers on both clients: 70 recordings, 244,000 ticks, 99.7% of them replayed exactly (243,241 of 243,988). Knockback, push, teleport, ice, soul sand, sneak edges, poses, epsilon moves, multi-system cases,
