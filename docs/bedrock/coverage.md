...
| monsters, 1.26.20.4 (gamepad) | 6 of 3203 | a hand-played fight: a few item uses the replay rebuilds from the packets (an eat that completes and goes on while the use key is held) |
| happy_ghast, 1.26.20.4 (gamepad) | 303 of 1850 | riding the happy ghast, which the server moves: the rider's seat (see above) |
| dragon, 1.26.20.4 (gamepad) | 325 of 5003 | eats the replay rebuilds from the packets, and after each such tick the float32 unit the box the replay rebuilds is off the client's (the client's own box has drifted from 0.6 wide as it moved, which the replay keeps where it can) |
