...
| `control` | `forward back left right jump sneak sprint` are the key levels; `raw` any other key bit (`ascend`, `descend`, `wantUpSlow`, `sneakToggleDown`, the diagonals, the jump/sneak edges); `analogMoveVector` a stick `{x, z}`; `moveVector` an already cooked move; `inputMode` the device (`'mouse'`, the default, `'game_pad'` or `'touch'`); `persistSneak` a gamepad's toggle sneak, kept through an input clear; `screen` a screen open, which clears the input. mineflayer sets the last three from `bot.bedrockInputMode`, `bot.bedrockPersistSneak` and `bot.currentWindow` |
